...
'Views' are the different states displayed in the UI. Views are generated by mixing model data (viewmodel) with a template. The template framework used by this tool is [handlebars][handlebars]. Templates must be embedded in the page as custom scripts. It is possible to use [server-side includes (SSI)][SSI] on the www.qld.gov.au environment in order to manage templates as separate files.
